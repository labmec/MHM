...
for(int k=1; k<2; k++){
    ...
    Configuration.pOrderInternal = pOrder_skel + n;
...
//    }
    ...
    SolveProblem(MHM->CMesh(), MHM->GetMeshes(), example, "MHMElast_H1", Configuration);
}//fim_j
}//fim_k
         }//fim_n
...
//    int dim = cmesh.Dimension();
    ...
    if(elast_example)
    {
        if (elast_example->fPlaneStress) {
            material1->SetPlaneStress();
        }
        material1->SetElasticityFunction(elast_example->ConstitutiveLawFunction());
    }
